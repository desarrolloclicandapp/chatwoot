class WaflowAgents::AutoReplyJob < ApplicationJob
  ...
  def perform(message_id)
    ...
    Rails.logger.info(
      "[WaflowAgents::AutoReplyJob] executing account=#{message.account_id} " \
      "inbox=#{message.inbox_id} conversation=#{message.conversation_id} message=#{message.id} agent=#{agent_id}"
    )
